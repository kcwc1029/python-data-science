# -*- coding: utf-8 -*-
"""範例 19：Seaborn第一張餐費圖。詳解請搭配折線圖 Notebook。"""
...
df = read_data("03_一個月餐費.csv", "日期")
sns.set_theme(style="whitegrid", font=font, rc={"axes.unicode_minus": False})
fig, ax = plt.subplots()
sns.lineplot(data=df, x="日期", y="餐費", estimator=None, errorbar=None, marker="o", ax=ax)
ax.set(title="Seaborn：每天的實際餐費", xlabel="日期", ylabel="餐費（元）")
date_axis(ax)
plt.show()
