# -*- coding: utf-8 -*-
"""範例 09：漏記餐費不是零元。詳解請搭配折線圖 Notebook。"""
...
calendar = pd.date_range("2026-04-01", "2026-04-06", freq="D")
complete = df.set_index("日期").reindex(calendar)
complete.index.name = "日期"
print(complete)
fig, ax = plt.subplots()
ax.plot(complete.index, complete["餐費"], marker="o", label="已知個人支出；缺漏處斷線")
ax.set(title="沒記錄與確實沒支出，要分開", xlabel="日期", ylabel="餐費（元）")
ax.set_ylim(bottom=0)
ax.legend()
date_axis(ax)
plt.show()
print("缺漏天數：", complete["餐費"].isna().sum())
