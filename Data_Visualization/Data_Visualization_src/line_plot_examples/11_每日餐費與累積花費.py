# -*- coding: utf-8 -*-
"""範例 11：每日餐費與累積花費。詳解請搭配折線圖 Notebook。"""
...
df["累積餐費"] = df["餐費"].cumsum()
fig, axes = plt.subplots(2, 1, figsize=(10, 7), sharex=True, layout="constrained")
axes[0].plot(df["日期"], df["餐費"], marker="o")
axes[0].set(title="今天吃多少？", ylabel="每日餐費（元）")
axes[1].plot(df["日期"], df["累積餐費"], color="#D55E00")
axes[1].axhline(6000, linestyle="--", color="gray", label="示範月預算 6,000 元")
axes[1].set(title="到今天累積吃多少？", xlabel="日期", ylabel="累積餐費（元）")
axes[1].legend()
date_axis(axes[1])
plt.show()
