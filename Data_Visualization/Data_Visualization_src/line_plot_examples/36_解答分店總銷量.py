# -*- coding: utf-8 -*-
"""範例 36：解答分店總銷量。詳解請搭配折線圖 Notebook。"""
...
daily = df.groupby(["日期", "分店"], as_index=False)["份數"].sum()
assert daily["份數"].sum() == df["份數"].sum()
assert not daily.duplicated(["日期", "分店"]).any()
fig, ax = plt.subplots()
sns.lineplot(data=daily, x="日期", y="份數", hue="分店", style="分店", markers=True,
             estimator=None, errorbar=None, ax=ax)
ax.set(title="每店每日兩品項總銷量", xlabel="日期", ylabel="總份數（份／日）")
date_axis(ax)
fig.savefig(OUT / "36_分店總銷量同圖.png", dpi=160, bbox_inches="tight")
plt.show()
g = sns.relplot(data=daily, x="日期", y="份數", col="分店", kind="line", marker="o",
                estimator=None, errorbar=None, height=3.3, aspect=1.05, facet_kws={"sharey": True})
g.set_axis_labels("日期", "總份數（份／日）")
g.set_titles("{col_name}")
for ax in g.axes.flat:
    date_axis(ax)
g.savefig(OUT / "36_分店總銷量分面.png", dpi=160)
plt.show()
print(daily.groupby("分店")["份數"].agg(["sum", "mean", "max"]).round(1))
