# -*- coding: utf-8 -*-
"""範例 27：通勤個體與平均一起看。詳解請搭配折線圖 Notebook。"""
...
part = df[df["出門時段"] == "07:10"]
fig, ax = plt.subplots()
sns.lineplot(data=part, x="日期", y="通勤分鐘", units="同學", estimator=None,
             errorbar=None, color="gray", alpha=0.45, linewidth=1, ax=ax)
sns.lineplot(data=part, x="日期", y="通勤分鐘", estimator="mean", errorbar=None,
             color="#0072B2", linewidth=3, marker="o", label="四人每日平均", ax=ax)
ax.set(title="07:10 出門組：灰線每人一條，藍線為平均", xlabel="日期", ylabel="通勤（分鐘）")
date_axis(ax)
plt.show()
