# -*- coding: utf-8 -*-
"""範例 23：多人通勤先看每天平均。詳解請搭配折線圖 Notebook。"""
...
summary = df.groupby(["日期", "出門時段"], as_index=False).agg(平均分鐘=("通勤分鐘", "mean"), 人數=("同學", "nunique"))
print(summary.head(6))
fig, ax = plt.subplots()
sns.lineplot(data=df, x="日期", y="通勤分鐘", hue="出門時段", style="出門時段",
             estimator="mean", errorbar=None, markers=True, ax=ax)
ax.set(title="每點為該日、該時段四位同學的平均", xlabel="日期", ylabel="平均通勤（分鐘）")
date_axis(ax)
plt.show()
