# -*- coding: utf-8 -*-
"""範例 29：Seaborn缺值連線的陷阱。詳解請搭配折線圖 Notebook。"""
...
df = read_data("05_缺漏餐費.csv", "日期")
complete = df.set_index("日期").reindex(pd.date_range("2026-04-01", "2026-04-06"))
complete.index.name = "日期"
complete = complete.reset_index()
fig, axes = plt.subplots(1, 2, figsize=(12, 4), sharey=True, layout="constrained")
sns.lineplot(data=complete, x="日期", y="餐費", estimator=None, errorbar=None, marker="o", ax=axes[0])
axes[0].set_title("直接交給 Seaborn：可能跨缺口連接")
axes[1].plot(complete["日期"], complete["餐費"], marker="o")
axes[1].set_title("完整日期交給 Matplotlib：NaN 保留斷線")
for ax in axes:
    ax.set(xlabel="日期", ylabel="餐費（元）", ylim=(0, 300))
    date_axis(ax)
plt.show()
