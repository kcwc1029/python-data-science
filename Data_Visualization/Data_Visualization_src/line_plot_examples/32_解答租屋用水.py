# -*- coding: utf-8 -*-
"""範例 32：解答租屋用水。詳解請搭配折線圖 Notebook。"""
...
df = read_data("12_練習租屋用水.csv")
fig, ax = plt.subplots()
for house, marker in [("甲屋", "o"), ("乙屋", "s")]:
    ax.plot(df["週次"], df[house], marker=marker, label=house)
ax.set(title="兩人租屋處的每週用水紀錄", xlabel="週次", ylabel="用水量（立方公尺／週）")
ax.legend()
ax.grid(axis="y", alpha=0.3)
plt.show()
print(df[["甲屋", "乙屋"]].agg(["mean", "max"]).round(2))
for house in ["甲屋", "乙屋"]:
    print(house, "最高用量的週次：", df.loc[df[house] == df[house].max(), "週次"].tolist())
print("用水升高值得檢查，但仍需確認訪客、清潔、抄表與漏水等因素。")
