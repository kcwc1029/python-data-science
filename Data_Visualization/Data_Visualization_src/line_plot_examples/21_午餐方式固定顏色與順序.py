# -*- coding: utf-8 -*-
"""範例 21：午餐方式固定顏色與順序。詳解請搭配折線圖 Notebook。"""
...
fig, ax = plt.subplots()
sns.lineplot(data=long, x="日期", y="費用", hue="方式", style="方式", hue_order=methods,
             style_order=methods, palette=colors, markers=True, estimator=None, errorbar=None, ax=ax)
ax.set(title="午餐費用情境比較：自備未計工時", xlabel="日期", ylabel="費用（元／餐）")
date_axis(ax)
plt.show()
