# -*- coding: utf-8 -*-
"""範例 24：平均與標準差陰影。詳解請搭配折線圖 Notebook。"""
...
fig, ax = plt.subplots()
sns.lineplot(data=df, x="日期", y="通勤分鐘", hue="出門時段", estimator="mean",
             errorbar="sd", marker="o", ax=ax)
ax.set(title="線是平均；陰影是平均 ± 1 個標準差", xlabel="日期", ylabel="通勤（分鐘）")
date_axis(ax)
plt.show()
one = df[(df["日期"] == df["日期"].min()) & (df["出門時段"] == "07:10")]["通勤分鐘"]
print("第一天 07:10 原始值：", one.tolist())
print("平均：", one.mean(), "樣本標準差：", round(one.std(), 2))
