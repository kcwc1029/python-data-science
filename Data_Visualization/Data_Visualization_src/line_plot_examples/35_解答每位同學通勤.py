# -*- coding: utf-8 -*-
"""範例 35：解答每位同學通勤。詳解請搭配折線圖 Notebook。"""
...
df = read_data("08_同學通勤紀錄.csv", "日期")
assert not df.duplicated(["同學", "日期"]).any()
times = sorted(df["出門時段"].unique())
colors = dict(zip(times, sns.color_palette("colorblind", len(times))))
fig, ax = plt.subplots()
sns.lineplot(data=df, x="日期", y="通勤分鐘", hue="出門時段", palette=colors,
             units="同學", estimator=None, errorbar=None, alpha=0.35, linewidth=1, ax=ax)
sns.lineplot(data=df, x="日期", y="通勤分鐘", hue="出門時段", palette=colors,
             estimator="mean", errorbar=None, linewidth=3, legend=False, ax=ax)
ax.set(title="淡線：每位同學；粗線：各時段當日平均", xlabel="日期", ylabel="通勤（分鐘）")
date_axis(ax)
plt.show()
print("紀錄列數：", len(df), "；不同同學：", df["同學"].nunique())
