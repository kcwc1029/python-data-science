# -*- coding: utf-8 -*-
"""範例 34：解答午餐長表。詳解請搭配折線圖 Notebook。"""
...
assert not long.duplicated(["日期", "方式"]).any()
fig, ax = plt.subplots()
sns.lineplot(data=long, x="日期", y="費用", hue="方式", style="方式", markers=True,
             estimator=None, errorbar=None, ax=ax)
ax.set(title="三種午餐取得方式（自備未計工時）", xlabel="日期", ylabel="元／餐")
date_axis(ax)
plt.show()
print(long.groupby("方式")["費用"].mean().round(1))
