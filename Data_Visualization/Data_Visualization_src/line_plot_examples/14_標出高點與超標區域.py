# -*- coding: utf-8 -*-
"""範例 14：標出高點與超標區域。詳解請搭配折線圖 Notebook。"""
...
budget = 250
peak = df.loc[df["餐費"].idxmax()]
fig, ax = plt.subplots()
ax.plot(df["日期"], df["餐費"], marker="o", label="每日餐費")
ax.axhline(budget, color="gray", linestyle="--", label="每日預算 250 元")
ax.fill_between(df["日期"], df["餐費"], budget, where=df["餐費"] > budget,
                interpolate=True, alpha=0.18, color="#D55E00", label="連線高於預算的示意區")
ax.annotate(f"最高 {peak['餐費']:.0f} 元", xy=(peak["日期"], peak["餐費"]),
            xytext=(0, 25), textcoords="offset points", ha="center",
            arrowprops={"arrowstyle": "->"})
ax.margins(y=0.25)
ax.set(title="哪幾天需要回頭看記帳明細？", xlabel="日期", ylabel="餐費（元）")
date_axis(ax)
ax.legend(loc="upper left", fontsize=9)
plt.show()
print("超支總額：", (df["餐費"] - budget).clip(lower=0).sum(), "元")
