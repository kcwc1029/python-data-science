# -*- coding: utf-8 -*-
"""範例 25：信賴區間不是個人通勤範圍。詳解請搭配折線圖 Notebook。"""
...
sns.lineplot(data=part, x="日期", y="通勤分鐘", errorbar="sd", marker="o", ax=axes[0])
sns.lineplot(data=part, x="日期", y="通勤分鐘", errorbar=("ci", 95), n_boot=1000,
             seed=42, marker="o", ax=axes[1])
axes[0].set_title("標準差：該日四人的分散程度")
axes[1].set_title("95% CI：平均估計的不確定性示範")
for ax in axes:
    ax.set(xlabel="日期", ylabel="通勤（分鐘）")
    date_axis(ax)
plt.show()
